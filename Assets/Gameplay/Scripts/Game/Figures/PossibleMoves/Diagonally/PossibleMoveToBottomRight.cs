...
public class PossibleMoveToBottomRight : PossibleMove
{
    Field[,] _fields;
    ...
    public override PossibleBasicMovementsModel Get(Team team, int2 point, PossibleBasicMovementsModel moves, int numberOfFields)
    {
        ...
        if (indexX + indexY > _fields.GetLength(0) - 1)     endIndex = math.clamp(numberOfFields - 1, int.MinValue, _fields.GetLength(0) - 1 - indexX);
        else                                                endIndex = math.clamp(numberOfFields - 1, int.MinValue, _fields.GetLength(0) - 1 - (_fields.GetLength(0) - 1 - indexY));

        if (indexX != _fields.GetLength(0) && indexY != -1)
